class Calculator {
  calculate(tokens) {
    return this.eval(this.parseTokens(tokens))
  }
  ...
  parseTokens(tokens) {
    ...
    if (tokenOperatorIdx > -1) {
      return {
        type: "operator",
        value: operators[i],
        left: this.parseTokens(tokens.slice(0, tokenOperatorIdx)),
        right: this.parseTokens(
  ...
  handleParenExpressions(tokens) {
    ...
    do {
      ...
      if (leftParenIdx > -1) {
        ...
        let expr = this.parseTokensParenExpr(
  ...
  parseTokensParenExpr(tokens) {
    return { type: "expr", subtree: this.parseTokens(tokens) }
  }
